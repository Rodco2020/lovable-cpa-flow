      ...
      <DialogContent className={`sm:max-w-${step === 'processing' || step === 'success' ? 'sm' : 'md'}`}>
        ...
        {step === 'confirmation' && (
          <div className="py-6 space-y-4">
            ...
            <div className="border rounded-md p-4 bg-gray-50">
              <p><strong>From:</strong> {sourceClientName}</p>
              <p><strong>To:</strong> {availableClients.find(client => client.id === targetClientId)?.legalName}</p>
              <div className="mt-2">
                <p><strong>Selected tasks:</strong></p>
                <ul className="mt-1 list-disc pl-5 text-sm">
                  {selectedAdHocTaskIds.size > 0 && (
        ...
        {step === 'success' && (
          <div className="py-6 space-y-6 text-center">
            ...
            <div>
              <h3 className="text-lg font-medium mb-1">Tasks Copied Successfully!</h3>
              <p className="text-sm text-gray-500">
                {copyResults ? (copyResults.recurring.length + copyResults.adHoc.length) : 0} task(s) have been copied to {availableClients.find(client => client.id === targetClientId)?.legalName}
              </p>
            </div>
        ...
        {/* Footer */}
        {(step === 'select-client' || step === 'select-tasks') && (
          <DialogFooter className="flex flex-col sm:flex-row sm:justify-between">
            <div>
              {step !== 'select-client' && (
                <Button variant="outline" onClick={handleBack} className="mt-2 sm:mt-0">
